...
interface Project {
  ...
  departments: { 主辦: string[], 協辦: string[] }  // 支援多單位
...
```
請實作 Milkdown WYSIWYG 編輯器：

技術堆疊：
- @milkdown/core: 核心編輯器
- @milkdown/preset-commonmark: Markdown 支援
- @milkdown/vue: Vue 3 整合
- @milkdown/plugin-listener: 內容變更監聽

特殊處理：
- 移除列表項目間的空行，保持文件精簡
- 同時支援 "-" 和 "*" 兩種 Markdown 標準列表符號
- 即時同步內容變更到預覽區
...
2. 專案詳情投影片 (Project)
   ...
   - 主辦/協辦單位
   - 甘特圖：
     - 支援自定義階段名稱（不限於開發/SIT/QAS/REG/PROD）
     - 各階段寬度依實際天數比例計算
     - 七個固定顏色循環：藍→綠→黃→粉紅→紫→橙→青
   - 今日標記（紅色垂直線）
   - 會辦狀況列表
...
5. 自定義時程階段
   - 支援任意階段名稱（如：需求分析、設計、UAT、正式上線）
   - 七個固定顏色循環（第八個階段回到第一個顏色）
   - 各階段寬度依實際天數比例顯示
```
